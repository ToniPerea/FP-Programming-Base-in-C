...
public:
   ...
   void Caracter_por_secuencia(char caracter_dado, Secuenciacaracteres secuencia_dada){
      int colocacion = secuencia_dada.Utilizados() - 1;
      int pos_a_borrar;

      for (int i = 0 ; i < utilizados ; i++){
         if (vector[i] == caracter_dado){
            pos_a_borrar = i;
            utilizados = utilizados + colocacion;

            for (int j = utilizados ; j > pos_a_borrar + colocacion ; j--){
               vector[j] = vector[j-colocacion];
            }
            for (int k = pos_a_borrar ; k < pos_a_borrar + secuencia_dada.Utilizados() ; k++){
               vector[k] = secuencia_dada.Get_Elemento(k-pos_a_borrar);
            }
