...
private:
   static const int ESPACIO = 2500000;
   char vector[ESPACIO];
   int utilizados = 0;
...
public:
   ...
   void Inserta_secuencia(string secuencia_dada, int posicion_dada){
      ...
      for (int i = 0 ; i < secuencia_dada.size() ; i++){
         for (int k = utilizados ; k > j ; k--)
            vector[k] = vector[k - 1];

         vector[j] = secuencia_dada[i];
         utilizados++;
         j++;
      }
   }

   void Caracter_por_secuencia(char caracter_dado, string secuencia_dada){
      int i = 0;

      while (i < utilizados){
         if (vector[i] == caracter_dado){
            Borrar_en_posicion(i);
            Inserta_secuencia(secuencia_dada, i);
            i = i + secuencia_dada.size() - 1;
         }
         else
            i++;
      }
...
int main(){
   ...
   string secuencia_para_remplazar;
   ...
   while (caracter != TERMINADOR){
      secuencia_para_remplazar.push_back(caracter);
      caracter = cin.get();
   }
